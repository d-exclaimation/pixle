...
export default rc(() => {
  ...
  return (
    <Dialog.Root open={isNewUser}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 z-50 bg-neutral-700/10 dark:bg-neutral-500/10 backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0" />
        <Dialog.Content
          className="fixed left-[50%] top-[50%] z-50 grid w-[95%] max-w-sm translate-x-[-50%] translate-y-[-50%] 
          gap-4 bg-neutral-100 dark:bg-neutral-800 p-6 duration-200 shadow
          border border-amber-600/50 dark:border-amber-500/50 rounded-lg
        ...
        >
          <Dialog.Title className="text-amber-700 dark:text-amber-200 m-0 text-[17px] font-medium">
            Pixle is experimental
          </Dialog.Title>
          <Dialog.Description className="text-amber-800 dark:text-amber-100 mt-[10px] mb-5 text-[15px] leading-normal">
          ...
          <div className="mt-[25px] flex justify-end">
            <button
              className="bg-green-600 dark:bg-green-200 text-green-100 md:dark:text-green-800 outline-none inline-flex h-[35px] items-center justify-center rounded-[4px] px-[15px] font-medium leading-none focus:shadow-[0_0_0_2px] focus:outline-none"
              onClick={confirm}
            >
              Continue
            </button>
